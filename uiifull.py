...
try:
  ...
  try:
    print(f'{NC} •    {R}ERROR {NC}=> {R}Something is Wrong{NC}..!!')
  finally:
    e = None
    del e
    sleep(1)
  ...
  exp = req.get('https://pastebin.com/raw/4Ey8NtKXi', verify=False).text
  ...
  def logins():
    ...
    handlew.close()
    lg()
    print(f'{NC} • =>{Y} Mencoba Untuk Login{NC}..!!')
    tungs(3)
    lg()
    print(f'{NC} • => {C}Login {G}Successed{NC}..!!')
    sleep(2)
  def loginf():
    print(f'{NC} • => {R}Kunci Salah{NC}..!!')
    print(f'{NC} • => {Y}Kunjungi Link Diatas{NC}..!!')
    sleep(2)
    kunci()
  def kunci():
    try:
      ...
      lg()
      print(f'{NC} •{G} Visit{NC} | {Y}{linkey}')
      inputpass = input(f'{NC} • {DW}Masukan Keys {NC}=> {DC}')
      if inputpass == '':
        print(f'{NC} • => {R}Input Kosong{NC}..!!')
        sleep(2)
        kunci()
      elif inputpass != keys:
        loginf()
      else:
        print(f'{NC} • => {G}Keys is Valid{NC}..!!')
        sleep(2)
        logins()
  ...
  def mm():
    ...
    try:
      ...
      if expired >= exp:
        lg()
        print(f' • {R}Waktu {C}Trial {Y}Habis{NC}..!!')
        sleep(1)
        print(f' • {Y}Hub Owner {NC}=> {R}WA {C}081233657831{NC} 🙏')
      else:
        lg()
